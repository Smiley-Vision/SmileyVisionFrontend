...
export function chooseRepresentativeProductItem(items) {
    ...
    return items.find((item) => getProductItemStock(item) > 0)
        ?? items[0]
}
...
export function formatLensValue(value) {
    ...
    return numericValue === 0 || Object.is(numericValue, -0)
        ? '0.00'
...
export function buildLensSeries(productItems, productImage = '') {
    ...
    for (const item of Array.isArray(productItems) ? productItems : []) {
        ...
        if (parsedLens) {
            ...
            group.cylinderMin = group.cylinderMin === null ? parsedLens.cylinder : Math.min(group.cylinderMin, parsedLens.cylinder)
            group.cylinderMax = group.cylinderMax === null ? parsedLens.cylinder : Math.max(group.cylinderMax, parsedLens.cylinder)
        }
    }
