...
const selectedProductItem = computed(() => {
    return productItems.value.find((item) => Number(item.id) === Number(selectedProductItemId.value)) ?? null
})

const productCategorySlug = computed(() => {
    const slug = getCategorySlug(product.value?.category?.name ?? product.value?.category_name ?? product.value?.category ?? '')
...
function getItemOfficeStock(item, officeId) {
    ...
    const inventoryRow = getItemInventoryRows(item).find((row) => (
        Number(row?.branch_office_id) === Number(officeId)
    ))
...
function getFrameVariationDetails(item) {
    const itemConfigurations = productConfigurations.value.filter((configuration) => (
        Number(configuration?.product_item_id) === Number(item?.id)
    ))
    ...
    return itemConfigurations
        ...
        .sort((left, right) => {
            ...
            return (order[normalizeVariationName(left.variationName)] ?? 99) -
...
async function retrieveData() {
    ...
    productItems.value = Array.isArray(productItemsResponse.data)
        ? productItemsResponse.data.filter((item) => Number(item?.product_id) === Number(product.value?.id))
        : []
...
<template>
    <div v-if="isLoading" class="flex flex-col mx-auto my-auto items-center gap-y-8 mt-24">
        <div class="text-sky-600 pi pi-spinner-dotted animate-spin slow-spin" style="font-size: 8rem"></div>
    ...
    >
        ...
        <div v-if="!hasProductItems" class="rounded-xl border border-amber-200 bg-amber-50 p-5 font-semibold text-amber-700">
                ...
                <div v-else class="grid gap-3 md:grid-cols-2">
                    ...
                    >
                        ...
                        <span class="flex min-w-0 flex-1 flex-col gap-1">
                            ...
                            <span class="text-sm font-semibold text-emerald-700">Total: {{ getItemTotalStock(item) }}</span>
                        </span>
                    </button>
                </div>
            </section>

            <section class="flex flex-col gap-5 rounded-xl border border-slate-200 bg-white p-5 shadow-lg">
                ...
                <div
                    :class="[
                        stockDelta > 0 ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : stockDelta < 0 ? 'border-amber-200 bg-amber-50 text-amber-700' : 'border-slate-200 bg-slate-50 text-slate-600',
                        'rounded-xl border p-4 text-sm font-semibold'
                    ]"
                >
                    {{ stockDelta === 0 ? 'Sin cambios pendientes.' : `Cambio pendiente: ${stockDelta > 0 ? '+' : ''}${stockDelta} unidades.` }}
                </div>
