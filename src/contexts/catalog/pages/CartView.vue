...
const addressLocation = computed(() => {
    ...
    const city = Number(defaultAddress.value.city_id) === 1
...
const addressMapUrl = computed(() => {
    if (!defaultAddress.value) return 'https://maps.google.com/maps?q=Campeche,Mexico&z=14&hl=es&output=embed'
...
async function loadDeliveryAddresses() {
    ...
    try {
        ...
        addresses.value = list.map((address) => ({
            ...address,
            is_default: address?.is_default === true || address?.is_default === 1 || address?.is_default === '1'
        }))
    } catch (error) {
        toast.add({
            severity: 'warn',
...
<template>
    ...
    <div v-else class="xl:px-20 md:px-12 px-8 lg:mt-14 md:py-10 py-8 flex flex-col gap-8">
        ...
        <div v-if="!cart.hasItems" class="bg-white border border-slate-200 rounded-2xl shadow-lg p-8 flex flex-col gap-6 items-start">
            <div class="text-2xl font-semibold text-sky-800">Tu carrito está vacío</div>
            <p class="text-slate-600 max-w-2xl">
                Aún no has agregado productos. Explora el catálogo para seleccionar equipos, micas o armazones.
            </p>
            <Button
        ...
        <div v-else class="grid xl:grid-cols-[1.65fr_1fr] grid-cols-1 gap-6">
            <section class="bg-white border border-slate-200 rounded-2xl shadow-lg p-6">
                ...
                <div class="flex flex-col divide-y divide-slate-200">
                    <article v-for="item in itemRows" :key="item.product_item_id" class="py-5 grid md:grid-cols-[7.5rem_1fr_auto] gap-4">
                        <img
