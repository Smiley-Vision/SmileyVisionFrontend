...
<template>
    ...
    <div v-else class="xl:px-20 md:px-12 px-8 lg:mt-14 md:py-10 py-8">
        ...
        <Card class="border border-slate-200 rounded-[20px] shadow-[0_8px_30px_rgba(14,88,128,0.1)]">
            <template #content>
                <div class="flex md:flex-row flex-col md:items-center items-start justify-between md:gap-6 gap-4 pb-6">
                    <div class="flex md:flex-row flex-col md:items-center items-start gap-5">
                        <div class="w-[124px] h-[124px] rounded-full flex items-center justify-center bg-[#d0dbe5]">
                            <i class="pi pi-user text-slate-500" style="scale: 3;"></i>
                        </div>
                        <div>
                            <div class="text-sky-800 md:text-5xl text-3xl font-bold leading-tight">{{ fullName }}</div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 border-y border-slate-200">
                    <div class="px-5 py-7">
                        <div class="text-[#075985] text-[1.35rem] md:text-[1.75rem] font-bold mb-[0.8rem]">Teléfono celular</div>
                    ...
                    <div class="px-5 py-7 border-t border-slate-200 md:border-t-0 md:border-l">
                        <div class="text-[#075985] text-[1.35rem] md:text-[1.75rem] font-bold mb-[0.8rem]">Correo electrónico</div>
                        <div class="flex items-center gap-[0.6rem] text-slate-600 text-[1.2rem] font-medium">
                ...
                <div class="pt-8">
                    ...
                    <div v-if="!hasAddresses" class="border border-dashed border-slate-400 rounded-xl p-4 text-slate-600 font-medium">
        ...
        <Dialog v-model:visible="showEditProfileModal" modal :style="{ width: '36rem' }">
            ...
            <form class="flex flex-col gap-4" @submit.prevent="submitProfileChanges">
                ...
                <div class="flex justify-end gap-3 mt-4">
                    <Button type="button" label="Cancelar" severity="secondary" text @click="showEditProfileModal = false" />
        ...
        <Dialog v-model:visible="showAddAddressModal" modal :style="{ width: '42rem' }">
            ...
            <form class="flex flex-col gap-4" @submit.prevent="submitCreateAddress">
                ...
                <div class="flex justify-end gap-3 mt-4">
                    <Button type="button" label="Cancelar" severity="secondary" text @click="showAddAddressModal = false" />
        ...
        <Dialog v-model:visible="showEditAddressModal" modal :style="{ width: '42rem' }">
            ...
            <form class="flex flex-col gap-4" @submit.prevent="submitEditAddress">
                ...
                <div class="flex justify-end gap-3 mt-4">
                    <Button type="button" label="Cancelar" severity="secondary" text @click="showEditAddressModal = false" />
        ...
        <Dialog v-model:visible="showDeleteAddressModal" modal :style="{ width: '30rem' }">
            ...
            <template #footer>
                <div class="flex justify-end gap-3">
                    <Button label="Cancelar" severity="secondary" text @click="showDeleteAddressModal = false" />
