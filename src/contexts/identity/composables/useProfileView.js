...
export function useProfileView() {
    ...
    async function loadAddresses() {
        ...
        const addressList = Array.isArray(response?.addresses)
            ? response.addresses
            : []
    ...
    async function submitProfileChanges() {
        try {
            ...
            toast.add({
                severity: 'warn',
                summary: 'Atención',
                detail: 'La API actual no expone endpoint para actualizar usuario. Cambio aplicado localmente.',
                life: 5000
            })
        } catch (error) {
            toast.add({
    ...
    async function submitEditAddress() {
        ...
        addresses.value = sortAddresses(
            addresses.value.map((address) => {
                ...
                return {
                    ...
                    internal_number: editAddressForm.internal_number === '' ? null : Number(editAddressForm.internal_number),
        ...
        toast.add({
            severity: 'warn',
            summary: 'Atención',
            detail: 'La API actual no expone endpoint para editar direcciones. Cambio visual solo en esta sesión.',
            life: 5500
        })
    }
    ...
    async function confirmDeleteAddress() {
        ...
        toast.add({
            severity: 'warn',
            summary: 'Atención',
            detail: 'La API actual no expone endpoint para eliminar direcciones. Cambio visual solo en esta sesión.',
            life: 5500
        })
    }
