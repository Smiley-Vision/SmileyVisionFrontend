...
<template>
  ...
  <div
    v-else
    class="flex flex-col xl:px-20 md:px-12 px-8 px-14 lg:mt-14 lg:py-0 md:py-10 py-8 lg:gap-y-16 gap-y-12"
  >
    <!-- Descriptive title and search bar -->
    <div class="flex lg:flex-row flex-col items-center lg:gap-y-0 gap-y-8 justify-between">
      <div class="lg:text-4xl text-3xl font-semibold text-sky-800">Catálogo de armazones</div>
      <RouterLink
        :to="{ name: 'shop-search-Armazones' }"
        class="flex lg:w-auto w-full bg-sky-200 hover:bg-sky-300 text-sky-800 px-6 py-3 rounded-xl shadow-xl"
      >
    ...
    <div class="flex justify-center">
      <div
        class="grid xl:grid-cols-4 sm:grid-cols-3 grid-cols-2 lg:gap-x-12 gap-x-8 lg:gap-y-10 gap-y-8 text-center"
      >
        <div v-for="(armazon, index) in armazones" :key="index">
          <div class="flex flex-col items-center gap-y-4">
            <RouterLink
              :to="{ name: 'product', params: { code: armazon.id } }"
              :productID="armazon.id"
            >
              <img
                :src="`${backendUrl}/storage/${armazon.image_url}`"
                class="lg:size-60 md:size-40 size-32 object-contain object-center bg-white border-solid border-4 border-sky-600 rounded-xl shadow-lg hover:shadow-2xl transform transition duration-200 hover:scale-105"
              />
            </RouterLink>
